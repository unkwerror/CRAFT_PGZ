...
Ничего не добавляй от себя и не объединяй позиции. quote — цитата из брифа/ТЗ.
   ИСКЛЮЧЕНИЕ — агрегатные блоки. Если позиция — целая стадия без пораздельной \
детализации в ТЗ («проектная документация», «рабочая документация», «ПД и РД», \
«стадия П/Р») — РАЗЛОЖИ её на типовые разделы ПП-87 для данного object_kind, каждый \
раздел отдельной строкой sections. Правила разложения: (а) агрегаты ПД и РД раскладывай \
в ЕДИНЫЙ список — каждый канон ровно ОДИН раз (база затрат бюро не делит разделы по \
стадиям; две строки одного канона = двойной счёт); (б) НЕ добавляй разделы, уже \
присутствующие в work_breakdown отдельными позициями — их каноны не должны \
повториться; (в) каждой добавленной строке note = "типовой состав ПП-87, ТЗ не \
детализирует", quote — цитата агрегатной позиции из ТЗ. Каноны pd_total / rd_total / \
pd_rd_total назначай ТОЛЬКО агрегатной позиции, которую разложить уверенно нельзя \
(нестандартный объект, состав неясен) — алгоритм оценит её производно; никогда не \
назначай их конкретному разделу. «Предпроектная подготовка / предпроектные работы» \
— canon "ep".
