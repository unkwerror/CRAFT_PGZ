...
ОКН-работы), затем сопоставимый масштаб цены. Для каждого — короткое reason.
...
def build_message(
    *,
    ...
    parts = [
        ...
        f"тендера. НМЦК для справки: {nmck:,.0f} ₽.".replace(",", " "),
        "=== КАРТОЧКА ЗАКУПКИ ===\n" + card_context,
...
def _parse_proposal(data: dict[str, Any], nmck: float) -> Proposal:
